...
export default {
  ...
  updatePost(parent, args, { db, pubsub }, info) {
    ...
    if (typeof data.published === 'boolean') {
      if (originalPost.published && !updPost.published) {
        pubsub.publish('post', {
          post: {
            mutation: 'DELETED',
            data: originalPost,
      ...
      } else if (!originalPost.published && updPost.published) {
        pubsub.publish('post', {
          post: {
            mutation: 'CREATED',
            data: updPost,
  ...
  createComment(parent, args, { db, pubsub }, info) {
    ...
    pubsub.publish(`comment ${newComment.post}`, {
      comment: {
        mutation: 'CRETED',
        data: newComment,
      },
